...
\title{\textbf{SPE Final Project}\\ \Large FraudGuard: An MLOps-Based Fraud Detection Platform}
\author{MT2025064 Kautilya Singh \\ MS2025021 Vishesh Tamrakar}
\date{\today}
...
\url{https://github.com/Vishesh-tamrakar/Fraudguard-devops-main}
\end{center}
...
\begin{itemize}
    ...
    \item \textbf{Automated CI/CD:} Fully automated testing, building, and deployment using Jenkins and GitHub Webhooks.
...
\begin{description}
    ...
    \item[\raisebox{-0.3\height}{\includegraphics[width=0.8cm]{Screenshots/logos/Jenkins.png}} \textbf{Jenkins:}] The automation engine orchestrating our CI/CD pipeline. It executes a declarative \texttt{Jenkinsfile} containing 7 discrete stages, completely eliminating manual deployment steps.
...
\node [block, right=of train] (eval) {Evaluate\\(ROC-AUC $> 0.85$)};
...
\begin{itemize}
    ...
    \item \textbf{Validation Constraint:} The training script mandates a minimum ROC-AUC score of 0.85 on the validation split. If this threshold is not met, the script fails, preventing a degraded model from being serialized.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Base Image:} Uses \texttt{python:3.11-slim} to minimize the final image size and reduce the attack surface.
...
The pipeline is fully automated and event-driven. A webhook is configured in the GitHub repository pointing to the Jenkins instance. Whenever a developer pushes a code commit to the \texttt{main} branch, GitHub immediately fires an HTTP payload to Jenkins, which instantly starts the build pipeline. This ensures the production environment is always in sync with the repository.
